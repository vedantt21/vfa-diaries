...
function App() {
    const [session, setSessionState] = useState(() => JSON.parse(localStorage.getItem("vfaSession") || "null"));
    ...
    const foodEntries = useMemo(() => sortFoodEntries(filteredEntries, foodSort), [filteredEntries, foodSort]);
...
function AuthScreen(props) {
    ...
    return e("section", { className: "auth-screen", "aria-label": "Account access" }, e("div", { className: "auth-copy" }, e(AuthBrandMark), e("div", { className: "auth-copy-block" }, e("p", { className: "section-kicker" }, "Food Diary"), e("h1", null, "VFA Diaries"), e("p", null, "Sign in to keep your restaurants, ratings, comments, and buy-again notes separate from everyone else."), e("p", { className: "crucible-badge" }, "backed by Crucible Ventures"))), e("div", { className: "auth-panel" }, showVerify
        ? e(VerifyForm, props)
        : e(AuthForm, props)));
}
function AuthBrandMark() {
    ...
    return e("div", { className: "auth-brand-mark", "aria-hidden": "true" }, colors.map((color, index) => e("span", {
        key: color,
        className: `brand-dot ${color} dot-${index + 1}`,
    })));
}
function AuthForm({ authMode, setAuthMode, authMessage, onAuthSubmit, }) {
    const isRegistering = authMode === "register";
    return e(React.Fragment, null, e("div", { className: "auth-tabs", role: "tablist", "aria-label": "Choose account mode" }, e("button", {
    ...
    }, "Create account")), e("form", { className: "auth-form", onSubmit: onAuthSubmit }, e("h2", null, isRegistering ? "Create your diary" : "Welcome back"), isRegistering &&
        ...
        e("label", null, "Username", e("input", {
            ...
            required: true,
        })), e("label", null, "Email", e("input", {
        name: "email",
        type: "email",
        autoComplete: "email",
        placeholder: "you@example.com",
        required: true,
    })), e("label", null, "Password", e("input", {
        name: "password",
        type: "password",
        autoComplete: isRegistering ? "new-password" : "current-password",
        minLength: 4,
        required: true,
    })), e("button", { className: "button", type: "submit" }, isRegistering ? "Create account" : "Log in"), e("p", { className: "message", role: "status" }, authMessage)));
}
function VerifyForm({ pendingEmail, verificationDelivery, verifyMessage, setPendingEmail, onVerifySubmit, onResend, }) {
    return e("form", { className: "auth-form", onSubmit: onVerifySubmit }, e("h2", null, "Verify your email"), e("p", { className: "auth-note" }, verificationDeliveryText(pendingEmail, verificationDelivery)), e("label", null, "Verification code", e("input", {
        name: "code",
        inputMode: "numeric",
        maxLength: 6,
        pattern: "[0-9]{6}",
        placeholder: "123456",
        required: true,
    })), e("div", { className: "verify-actions" }, e("button", { className: "button", type: "submit" }, "Verify"), e("button", { className: "button secondary", type: "button", onClick: onResend }, "Resend code")), e("button", {
        className: "button ghost",
        type: "button",
        onClick: () => setPendingEmail(""),
    }, "Back to login"), e("p", { className: "message", role: "status" }, verifyMessage));
}
function MainApp(props) {
    ...
    function focusFoodSearch() {
        openTab("food");
        setTimeout(() => {
            const input = document.querySelector("[data-food-search='true']");
            if (input) {
                input.focus();
            }
        }, 240);
    ...
    return e(React.Fragment, null, e("main", { className: `app-shell ${menuOpen ? "menu-open" : ""}` }, e(TopBar, {
        onHome: () => openTab("home"),
        onAdd: () => openTab("add"),
        ...
        onOpenTab: openTab,
    }), e("section", { className: "screen-hero" }, e("p", { className: "section-kicker" }, screen.eyebrow), e("h1", { className: "screen-title" }, screen.title), e("p", { className: "screen-copy" }, screen.copy), e("div", { className: "hero-metrics" }, screen.metrics.map((metric) => e(SocialCountPill, { key: metric.label, label: metric.label, value: metric.value })))), e("div", { className: "screen-content" }, props.appTab === "home" &&
        e(HomePanel, props), props.appTab === "food" &&
        e(FoodPanel, props), props.appTab === "restaurants" &&
        e(RestaurantsPanel, props), props.appTab === "add" &&
        e(AddFoodPanel, {
            status: props.status,
            onSaveEntry: props.onSaveEntry,
            suggestions: props.suggestions,
        }), props.appTab === "friends" &&
        e(FriendsPanel, { session: props.session, onRefreshProfile: props.onRefreshProfile }), props.appTab === "stats" &&
        e(StatsPanel, { stats: props.stats, setAppTab: props.setAppTab }))));
}
function SocialCountPill({ label, value }) {
    return e("span", { className: "profile-pill" }, e("strong", null, value), ` ${label}`);
}
function TopBar({ onHome, onAdd, onSearch, onToggleMenu }) {
    return e("header", { className: "top-bar" }, e(BrandDots, { onClick: onHome }), e("div", { className: "top-bar-actions" }, e(IconButton, { label: "Add food", icon: "plus", onClick: onAdd }), e(IconButton, { label: "Open menu", icon: "menu", onClick: onToggleMenu }), e(IconButton, { label: "Search food", icon: "search", onClick: onSearch })));
}
...
function TopBarIcon({ icon }) {
    if (icon === "menu") {
        return e("span", { className: "top-icon top-icon-menu", "aria-hidden": "true" }, e("span", null), e("span", null), e("span", null));
    }
    if (icon === "search") {
        return e("span", { className: "top-icon top-icon-search", "aria-hidden": "true" }, e("span", { className: "search-ring" }), e("span", { className: "search-handle" }));
    }
    return e("span", { className: "top-icon top-icon-plus", "aria-hidden": "true" }, e("span", null), e("span", null));
}
function SideMenu({ open, session, socialCounts, activeTab, onClose, onLogout, onOpenTab }) {
    ...
    return e(React.Fragment, null, e("button", {
        className: `menu-backdrop ${open ? "visible" : ""}`,
        type: "button",
        "aria-label": "Close menu",
        onClick: onClose,
    }), e("aside", { className: `side-menu ${open ? "open" : ""}` }, e("div", { className: "menu-top" }, e("div", { className: "menu-avatar" }, initialsFromName(session.user.displayName)), e("div", { className: "menu-profile-copy" }, e("p", { className: "menu-username" }, `@${session.user.username || "diary_user"}`), e("div", { className: "menu-social" }, e(SocialCountPill, { label: "Followers", value: socialCounts.followers }), e(SocialCountPill, { label: "Following", value: socialCounts.following }), e(SocialCountPill, { label: "Friends", value: socialCounts.friends })))), e("div", { className: "menu-divider" }), e("div", { className: "menu-grid" }, mainColumns.map((column, index) => e("div", { key: index, className: "menu-column" }, column.map((item) => e(MenuLink, {
        key: `${item.label}-${index}`,
        active: item.active,
        onClick: () => onOpenTab(item.tab),
    }, item.label))))), e("div", { className: "menu-divider" }), e("div", { className: "menu-secondary" }, e(MenuLink, { onClick: onLogout }, "Sign out")), e("div", { className: "menu-divider" }), e("div", { className: "menu-bottom" }, lowerLinks.map((item) => e(MenuLink, {
        key: item.label,
        active: item.active,
        onClick: () => onOpenTab(item.tab),
    }, item.label)))));
...
function SectionHeader({ label, title, actionLabel = "", onAction = null }) {
    return e("div", { className: "section-header" }, e("div", { className: "section-header-top" }, e("p", { className: "section-kicker" }, label), actionLabel &&
        e("button", {
            className: "section-action",
            type: "button",
            onClick: onAction || (() => { }),
        }, actionLabel)), e("h2", { className: "section-title" }, title), e("div", { className: "section-line", "aria-hidden": "true" }));
}
...
function StatsPanel({ stats, setAppTab }) {
    if (!stats.dishCount) {
        return e("section", { className: "app-panel active", "aria-label": "Diary stats" }, e("div", { className: "stats-page" }, e(SectionHeader, { label: "Stats", title: "Your stats" }), e("div", { className: "empty-state visible premium-empty-state" }, e("img", {
            className: "empty-logo",
            src: LOGO_SRC,
            alt: "",
            "aria-hidden": "true",
        }), e("div", null, e("h3", null, "No stats yet."), e("p", null, "Add a food note and this page will start filling up."), e("button", { className: "button secondary", type: "button", onClick: () => setAppTab("add") }, "Add food")))));
    }
    return e("section", { className: "app-panel active", "aria-label": "Diary stats" }, e("div", { className: "stats-page" }, e(SectionHeader, {
        label: "Stats",
        title: "Your stats",
        actionLabel: "Add food",
        onAction: () => setAppTab("add"),
    }), e("p", { className: "screen-subcopy" }, "A fast, premium read on where your best meals, spend, and repeat picks are clustering."), e("div", { className: "stats-summary" }, e(StatCard, {
        label: "Logged",
    ...
    })), e("div", { className: "stats-highlights" }, e(StatCard, {
        label: "Best bite",
    ...
    })), e("div", { className: "stats-breakdowns" }, e(BreakdownPanel, {
        title: "Top cuisines",
...
function StatCard({ label, value, detail }) {
    return e("article", { className: "stat-card" }, e("span", null, label), e("strong", { title: value }, value), e("p", null, detail));
}
function BreakdownPanel({ title, items, valueType = "count", emptyText }) {
    const maxValue = Math.max(1, ...items.map((item) => (valueType === "money" ? item.total : item.count)));
    return e("section", { className: "stats-panel" }, e("h3", null, title), items.length
        ? e("div", { className: "rank-list" }, items.map((item) => {
            const amount = valueType === "money" ? item.total : item.count;
            const value = valueType === "money" ? formatMoney(item.total) : String(item.count);
            return e("div", { className: "rank-row", key: item.label }, e("div", { className: "rank-line" }, e("span", { title: item.label }, item.label), e("strong", null, value)), e("div", { className: "rank-track", "aria-hidden": "true" }, e("span", { style: { width: `${amount ? Math.max(4, (amount / maxValue) * 100) : 0}%` } })), item.detail && e("p", null, item.detail));
        }))
        : e("p", { className: "stats-empty" }, emptyText));
}
function HomePanel({ session, homeEntries, restaurants, stats, hasAnyEntries, setAppTab, onDeleteEntry, }) {
    const [expandedId, setExpandedId] = useState(null);
    const topRestaurants = restaurants.slice(0, 4);
    if (!hasAnyEntries) {
        return e("section", { className: "app-panel active", "aria-label": "Home" }, e("div", { className: "panel-block home-panel-block" }, e(SectionHeader, {
            label: "Home",
            title: "Start your diary",
            actionLabel: "Add food",
            onAction: () => setAppTab("add"),
        }), e("div", { className: "empty-state visible premium-empty-state" }, e("img", {
            className: "empty-logo",
            src: LOGO_SRC,
            alt: "",
            "aria-hidden": "true",
        }), e("div", null, e("h3", null, "No food logged yet."), e("p", null, "Add your first restaurant note and your home feed will start filling up."), e("button", { className: "button secondary", type: "button", onClick: () => setAppTab("add") }, "Add food")))));
    }
    return e("section", { className: "app-panel active", "aria-label": "Home" }, e("div", { className: "panel-stack" }, e("div", { className: "panel-block home-panel-block" }, e(SectionHeader, {
        label: "New from diary",
        title: "Recent food posters",
        actionLabel: "See all food",
        onAction: () => setAppTab("food"),
    }), e("div", { className: "entries poster-grid" }, homeEntries.map((entry) => e(FoodCard, {
        key: entry.id,
        entry,
        ...
        onDeleteEntry,
    })))), e("div", { className: "panel-block home-panel-block" }, e(SectionHeader, {
        label: "Restaurants",
        title: "Most visited spots",
        actionLabel: "All restaurants",
        onAction: () => setAppTab("restaurants"),
    }), e("div", { className: "restaurant-grid" }, topRestaurants.map((restaurant) => e(RestaurantCard, { key: restaurant.key, restaurant })))), e("div", { className: "home-stats-strip" }, e(StatCard, {
        label: "Average rating",
        value: `${formatRating(stats.averageRating)} / 10`,
...
function FoodPanel({ session, search, setSearch, entryCount, entries, hasAnyEntries, setAppTab, onDeleteEntry, filterCuisine, setFilterCuisine, filterMinRating, setFilterMinRating, filterMaxRating, setFilterMaxRating, filterBuyAgain, setFilterBuyAgain, foodSort, setFoodSort, suggestions, }) {
    const [expandedId, setExpandedId] = useState(null);
    return e("section", { className: "app-panel active", "aria-label": "Food diary entries" }, e("div", { className: "panel-block diary-block" }, e(SectionHeader, {
        label: "Food",
        title: "Your full food library",
        actionLabel: `${entryCount} ${entryCount === 1 ? "entry" : "entries"}`,
    }), e("div", { className: "search-row premium-search-row" }, e("input", {
        type: "search",
        value: search,
    ...
    })), e("div", { className: "filters-row premium-filter-row" }, e("div", { className: "filter-group" }, e("label", null, "Sort"), e("select", {
        value: foodSort,
        onChange: (event) => setFoodSort(event.target.value),
        className: "filter-input",
    }, e("option", { value: "latest" }, "Latest"), e("option", { value: "rating_desc" }, "Highest rated"), e("option", { value: "rating_asc" }, "Lowest rated"), e("option", { value: "restaurant_az" }, "Restaurant A-Z"), e("option", { value: "dish_az" }, "Dish A-Z"), e("option", { value: "price_desc" }, "Most expensive"))), e("div", { className: "filter-group" }, e("label", null, "Cuisine"), e("select", {
        value: filterCuisine,
        onChange: (event) => setFilterCuisine(event.target.value),
        className: "filter-input",
    }, e("option", { value: "" }, "All cuisines"), suggestions.cuisines.map((cuisine) => e("option", { key: cuisine, value: cuisine }, cuisine)))), e("div", { className: "filter-group" }, e("label", null, "Min rating"), e("input", {
        type: "number",
        ...
        max: "10",
        className: "filter-input",
    })), e("div", { className: "filter-group" }, e("label", null, "Max rating"), e("input", {
        type: "number",
        ...
        max: "10",
        className: "filter-input",
    })), e("div", { className: "filter-group" }, e("label", null, "Buy again"), e("select", {
        value: filterBuyAgain,
        onChange: (event) => setFilterBuyAgain(event.target.value),
        className: "filter-input",
    }, e("option", { value: "" }, "All"), e("option", { value: "yes" }, "Yes"), e("option", { value: "no" }, "No"))), e("button", {
        ...
        ? e("div", { className: "empty-state visible premium-empty-state" }, e("img", {
            ...
            alt: "",
            "aria-hidden": "true",
        }), e("div", null, e("h3", null, "No food logged yet."), e("p", null, "Add your first restaurant note and it will stay here."), e("button", { className: "button secondary", type: "button", onClick: () => setAppTab("add") }, "Add food")))
        : entries.length === 0
            ? e("div", { className: "empty-state visible premium-empty-state" }, e("img", {
                className: "empty-logo",
                src: LOGO_SRC,
                alt: "",
                "aria-hidden": "true",
            }), e("div", null, e("h3", null, "No matches."), e("p", null, "Try a different food or restaurant name."), e("button", { className: "button secondary", type: "button", onClick: () => setSearch("") }, "Clear search")))
            : e("div", { className: "entries poster-grid" }, entries.map((entry) => e(FoodCard, {
                key: entry.id,
                entry,
                viewerUsername: session?.user?.username,
                isExpanded: expandedId === entry.id,
...
function RestaurantsPanel({ restaurants, hasAnyEntries, setAppTab }) {
    return e("section", { className: "app-panel active", "aria-label": "Restaurants" }, e("div", { className: "panel-block restaurants-panel-block" }, e(SectionHeader, {
        label: "Restaurants",
        ...
        ? e("div", { className: "empty-state visible premium-empty-state" }, e("img", {
            ...
            alt: "",
            "aria-hidden": "true",
        }), e("div", null, e("h3", null, "No restaurants yet."), e("p", null, "Once you add a food note, every restaurant you visit will show up here."), e("button", { className: "button secondary", type: "button", onClick: () => setAppTab("add") }, "Add food")))
        : e("div", { className: "restaurant-directory" }, restaurants.map((restaurant) => e(RestaurantCard, { key: restaurant.key, restaurant, detailed: true })))));
}
function RestaurantCard({ restaurant, detailed = false }) {
    return e("article", { className: `restaurant-card ${detailed ? "detailed" : ""}` }, e("div", { className: "restaurant-card-top" }, e("div", { className: "restaurant-card-copy" }, e("p", { className: "restaurant-kicker" }, restaurant.primaryCuisine || "Restaurant"), e("h3", { className: "restaurant-title" }, restaurant.name), e("p", { className: "restaurant-subtitle" }, restaurant.suburb || "Location not logged")), e("div", { className: "restaurant-rating" }, e("strong", null, `${formatRating(restaurant.averageRating)}/10`), e("div", { className: "star-row", "aria-hidden": "true" }, ratingStars(restaurant.averageRating)))), e("div", { className: "restaurant-meta" }, e("span", { className: "meta-item" }, `${restaurant.visits} ${restaurant.visits === 1 ? "visit" : "visits"}`), restaurant.latestDish && e("span", { className: "meta-item" }, restaurant.latestDish), restaurant.latestDate && e("span", { className: "meta-item" }, `Last ${formatSocialDate(restaurant.latestDate)}`), restaurant.buyAgainRate > 0 &&
        e("span", { className: "meta-item" }, `${Math.round(restaurant.buyAgainRate * 100)}% buy again`)), detailed &&
...
function FoodCard({ entry, viewerUsername, isExpanded, onToggle, onDeleteEntry }) {
    ...
    }, e("div", { className: "poster-art", style: posterArtStyle(entry) }, e("span", { className: "poster-kicker" }, entry.cuisine || entry.suburb || entry.restaurant), e("h3", { className: "poster-title" }, entry.dish), e("p", { className: "poster-subtitle" }, entry.restaurant), e("span", { className: "poster-score" }, `${formatRating(entry.rating)}/10`)), e("div", { className: "poster-body" }, e("div", { className: "poster-meta-row" }, e("p", { className: "card-username" }, `@${username}`), e("div", { className: "star-row", "aria-label": `${formatRating(entry.rating)} out of 10` }, stars)), e("div", { className: "entry-meta" }, entry.suburb && e("span", { className: "meta-item" }, entry.suburb), entry.price !== null &&
        entry.price !== undefined &&
        e("span", { className: "meta-item" }, formatMoney(entry.price)), e("span", { className: "meta-item" }, entry.wouldBuyAgain ? "Buy again" : "One-time"), e("span", { className: "meta-item" }, visibilityLabel(entry.visibility))), isExpanded && entry.comments && e("p", { className: "entry-comments" }, entry.comments)), isExpanded &&
        e("button", {
            className: "button ghost",
            type: "button",
            onClick: (event) => {
...
function AddFoodPanel({ status, onSaveEntry, suggestions }) {
    ...
    return e("section", { className: "app-panel active", "aria-label": "Add food" }, e("div", { className: "panel-block form-block add-food-block" }, e(SectionHeader, {
        label: "Capture",
        title: "Add a food note",
    }), e("p", { className: "screen-subcopy" }, "A clean one-pass form for the dish, the place, the score, and whether it deserves a return trip."), e("form", { className: "form-stack premium-form-stack", onSubmit: submit }, e(Datalist, { id: "restaurant-options", options: suggestions.restaurants }), e(Datalist, { id: "suburb-options", options: suggestions.suburbs }), e(Datalist, { id: "dish-options", options: suggestions.dishes }), e(Datalist, { id: "cuisine-options", options: suggestions.cuisines }), e(FormField, {
        ...
        step: "0.01",
        min: "0",
    }), e("label", { className: "question-block rating-field" }, e("span", { className: "label-row" }, "Rating", e("span", { className: "hint" }, `${formatRating(rating)} / 10`)), e("input", {
        name: "rating",
        type: "number",
        min: "0",
        max: "10",
    ...
    })), e("label", { className: "question-block" }, "Would you buy again?", e("select", { name: "wouldBuyAgain", required: true }, e("option", { value: "yes" }, "Yes"), e("option", { value: "no" }, "No"))), e("label", { className: "question-block" }, "Who can see this?", e("select", { name: "visibility", required: true }, e("option", { value: "private" }, "🔒 Private (only me)"), e("option", { value: "friends_only" }, "👥 Friends only (my followers)"), e("option", { value: "public" }, "🌐 Public (everyone)"))), e("label", { className: "question-block" }, "Comments", e("textarea", {
        name: "comments",
        maxLength: 600,
        placeholder: "Worth it? Too salty? Good value?",
    })), e("div", { className: "actions" }, e("button", { className: "button", type: "submit" }, "Add food"), e("button", {
        className: "button secondary",
        type: "reset",
        onClick: () => setRating(DEFAULT_RATING),
    }, "Clear"))), e("p", { className: "message", role: "status" }, status)));
}
function FormField({ label, className = "", ...props }) {
    return e("label", { className }, label, e("input", props));
}
function FriendsPanel({ session, onRefreshProfile }) {
    ...
    async function followUser(username) {
        await performRelationshipAction(username, "follow", "Relationship updated.", (data) => {
            setSearchResults((current) => current.map((user) => user.username === username
                ? {
                    ...user,
    ...
    async function acceptFollowRequest(username) {
        const data = await performRelationshipAction(username, "accept", "Request accepted.");
        if (!data) {
            return;
        }
    ...
    async function rejectFollowRequest(username) {
        const data = await performRelationshipAction(username, "reject", "Request removed.");
        if (!data) {
            return;
        }
        await loadRelationshipList("pending", setPendingRequests);
    ...
    async function unfollowUser(username) {
        const data = await performRelationshipAction(username, "unfollow", "You are no longer following them.", () => {
            setSearchResults((current) => current.map((user) => user.username === username
    ...
    async function removeFollower(username) {
        const data = await performRelationshipAction(username, "remove", "Follower removed.", () => {
            setSearchResults((current) => current.map((user) => user.username === username
    ...
    return e("section", { className: "app-panel active", "aria-label": "Friends and activity" }, e("div", { className: "panel-block friends-panel-block" }, e(SectionHeader, {
        label: "Social",
        title: "Friends and activity",
        actionLabel: "All activity",
        onAction: () => setActiveTab("activity"),
    }), e("p", { className: "friends-intro screen-subcopy" }, "Build your own food-circle: follow people, follow back, and keep up with what they rate."), e("div", { className: "friends-summary-grid" }, e(SocialSummaryCard, { label: "Followers", value: socialCounts.followersCount }), e(SocialSummaryCard, { label: "Following", value: socialCounts.followingCount }), e(SocialSummaryCard, { label: "Friends", value: socialCounts.friendsCount }), e(SocialSummaryCard, { label: "Requests", value: socialCounts.pendingCount })), e("div", { className: "friends-tabs" }, e(FriendsTabButton, {
    ...
    })), e("p", { className: "message friends-message", role: "status" }, panelStatus), activeTab === "activity" &&
        e("div", { className: "friends-content" }, loading && e("p", { className: "empty-message" }, "Loading activity..."), !loading &&
            (feedEntries.length > 0
                ? e("div", { className: "feed-list poster-grid" }, feedEntries.map((entry) => e(FeedPosterCard, { key: `${entry.id}-${entry.username}`, entry })))
                : e("p", { className: "empty-message" }, "No activity yet. Follow a few people to get their public and friends-only food notes here."))), activeTab === "discover" &&
        e("div", { className: "friends-content" }, e("form", { onSubmit: handleSearch, className: "search-form" }, e("input", {
            type: "text",
            placeholder: "Search by username or display name...",
            value: searchTerm,
            onChange: (event) => setSearchTerm(event.target.value),
            className: "filter-input",
        }), e("button", { type: "submit", className: "button", disabled: loading }, loading ? "Searching..." : "Search")), searchResults.length > 0
            ? e("div", { className: "search-results" }, searchResults.map((user) => e(SocialUserCard, {
                key: user.username,
                user,
            ...
            : e("p", { className: "empty-message" }, "Search for someone to start building your food network.")), activeTab === "requests" &&
        e("div", { className: "friends-content" }, loading && e("p", { className: "empty-message" }, "Loading requests..."), !loading &&
            (pendingRequests.length > 0
                ? pendingRequests.map((user) => e(SocialUserCard, {
                    key: user.username,
                    user,
                    actionArea: e("div", { className: "request-actions" }, e("button", {
                        className: "button",
                        type: "button",
                        disabled: activeAction === `accept:${user.username}`,
                        onClick: () => acceptFollowRequest(user.username),
                    }, activeAction === `accept:${user.username}` ? "Accepting..." : "Accept"), e("button", {
                        className: "button secondary",
                        type: "button",
                ...
                : e("p", { className: "empty-message" }, "No pending requests."))), activeTab === "followers" &&
        e("div", { className: "friends-content" }, loading && e("p", { className: "empty-message" }, "Loading followers..."), !loading &&
            (followers.length > 0
                ? followers.map((user) => e(SocialUserCard, {
                    key: user.username,
                    user,
                    actionArea: e("div", { className: "request-actions" }, user.followStatus === "accepted"
                        ? e("button", {
                            className: "button secondary",
                            type: "button",
                        ...
                        }, activeAction === `follow:${user.username}` ? "Following..." : "Follow back"), e("button", {
                        className: "button secondary",
                        type: "button",
                ...
                : e("p", { className: "empty-message" }, "No followers yet."))), activeTab === "following" &&
        e("div", { className: "friends-content" }, loading && e("p", { className: "empty-message" }, "Loading following..."), !loading &&
            (following.length > 0
                ? following.map((user) => e(SocialUserCard, {
                    key: user.username,
                    user,
                    actionArea: e("button", {
                        className: "button secondary",
                ...
                : e("p", { className: "empty-message" }, "You are not following anyone yet."))), activeTab === "friends" &&
        e("div", { className: "friends-content" }, loading && e("p", { className: "empty-message" }, "Loading friends..."), !loading &&
            (friends.length > 0
                ? friends.map((user) => e(SocialUserCard, {
                    key: user.username,
                    user,
                    actionArea: e("div", { className: "request-actions" }, e("button", {
                        className: "button secondary",
                        ...
                        disabled: activeAction === `unfollow:${user.username}`,
                        onClick: () => unfollowUser(user.username),
                    }, activeAction === `unfollow:${user.username}` ? "Updating..." : "Unfollow"), e("button", {
                        className: "button secondary",
                        type: "button",
                        disabled: activeAction === `remove:${user.username}`,
                        onClick: () => removeFollower(user.username),
...
function SocialSummaryCard({ label, value }) {
    return e("article", { className: "friends-summary-card" }, e("p", null, label), e("strong", null, value));
}
function SocialUserCard({ user, actionArea }) {
    return e("div", { className: "social-user-card" }, e("div", { className: "social-user-copy" }, e("strong", null, user.displayName), e("p", { className: "username" }, `@${user.username}`), e("p", { className: "social-user-meta" }, socialUserMeta(user)), e("div", { className: "social-badges" }, relationshipBadgeText(user).map((badge) => e("span", { key: badge, className: "social-badge" }, badge)))), actionArea);
}
function FeedPosterCard({ entry }) {
    return e("article", { className: "feed-card poster-card" }, e("div", { className: "poster-art", style: posterArtStyle(entry) }, e("span", { className: "poster-kicker" }, entry.cuisine || entry.suburb || "Friend activity"), e("h3", { className: "poster-title" }, entry.dish), e("p", { className: "poster-subtitle" }, entry.restaurant), e("span", { className: "poster-score" }, `${formatRating(entry.rating)}/10`)), e("div", { className: "poster-body" }, e("div", { className: "poster-meta-row" }, e("div", null, e("p", { className: "card-username" }, `@${entry.username}`), e("p", { className: "feed-timestamp" }, formatSocialDate(entry.createdAt))), e("div", { className: "star-row", "aria-hidden": "true" }, ratingStars(entry.rating))), e("div", { className: "entry-meta feed-meta" }, e("span", { className: "meta-item" }, visibilityLabel(entry.visibility)), entry.price !== null &&
        entry.price !== undefined &&
        e("span", { className: "meta-item" }, formatMoney(entry.price))), entry.comments && e("p", { className: "entry-comments" }, entry.comments)));
}
...
function Datalist({ id, options }) {
    return e("datalist", { id }, options.map((option) => e("option", { key: option, value: option })));
}
...
function ratingStars(value) {
    ...
    return Array.from({ length: 5 }, (_, index) => e("span", {
        key: index,
...
function buildRestaurantDirectory(entries) {
    ...
    return Array.from(restaurants.values())
        .map((restaurant) => {
        const cuisines = Array.from(restaurant.cuisineCounts.entries()).sort((first, second) => second[1] - first[1] || first[0].localeCompare(second[0]));

...
def load_env_file(path: Path = ROOT / ".env", override: bool = False) -> None:
    if not path.is_file():
        return
...
def database_url() -> str:
    ...
    if not configured_url:
        configured_url = os.environ.get("POSTGRES_URL", "").strip()
...
def init_postgres_db() -> None:
    with db() as conn:
        ...
        conn.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS verification_salt TEXT")
        conn.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS verification_hash TEXT")
        conn.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS verification_expires_at TEXT")
...
def insert_user(conn, params: tuple) -> int:
    sql = """
        INSERT INTO users (
            display_name,
            username,
...
def insert_entry(conn, params: tuple) -> int:
    sql = """
        INSERT INTO entries (
            user_id,
            restaurant,
...
class VfaHandler(BaseHTTPRequestHandler):
    ...
    def list_entries(self, query: str) -> None:
        ...
        """
        order_expression = (
            "entries.created_at" if database_url() else "datetime(entries.created_at)"
        )
        sql += f" ORDER BY {order_expression} DESC, entries.id DESC"
    ...
    def save_entry(self) -> None:
        ...
        try:
            ...
            suburb = self.clean_text(self.form_value(form, "suburb"), 80)
...
def run() -> None:
    ...
    if database_url():
        print("Database: Postgres from DATABASE_URL")
    else:
